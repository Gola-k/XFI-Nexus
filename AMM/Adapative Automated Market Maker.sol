...
function swap(uint256 inputAssetIndex, uint256 outputAssetIndex, uint256 inputAmount, uint256 minOutputAmount) external nonReentrant whenNotPaused returns (uint256) {
    ...
    function calculateOutputAmount(uint256 inputAssetIndex, uint256 outputAssetIndex, uint256 inputAmount) public view returns (uint256) {
        ...
        uint256 inputPrice = getLatestPrice(inputAssetIndex);
        uint256 outputPrice = getLatestPrice(outputAssetIndex);

        console.log("Input Price:", inputPrice);
        console.log("Output Price:", outputPrice);

        uint256 inputValue = (inputAmount * inputPrice) / 1e8;  
        console.log("Input Value:", inputValue);

        uint256 outputValue = (inputValue * outputAsset.weight) / inputAsset.weight;
        console.log("Output Value:", outputValue);

        uint256 outputAmount = (outputValue * 1e8) / outputPrice;
    ...
    address assetAddress = address(assets[assetIndex].token);  // Get the token address
    uint256 price = assets[assetIndex].priceFeed.getPrice(assetAddress);  // Pass the token address to getPrice
    return price;
}
